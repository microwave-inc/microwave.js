...
module.exports.interaction = async (interaction, client) => {
    ...
    var api = await fetch(`https://apod.ellanan.com/api`); // Set the API URL so shit stops complaining
    ...
    /*
        if (year != null) {
            if (month != null) {
                if (day != null) {
                    let api = `https://apod.ellanan.com/api?&date=${year}-${month}-${day}`;
            ...
            let api = `https://apod.ellanan.com/api`
        }
    } else {   
        let api = `https://apod.ellanan.com/api`;
    }
    ...
    */
    ...
    if (api.media_type === "video") { // Check if they are using an image or video and set it accordingly.
        embed.video(api.url)
    } else {
